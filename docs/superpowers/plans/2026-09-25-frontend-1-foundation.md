...
/* 这两条必须放进 @layer base。Tailwind v4 把工具类放在 `@layer utilities` 里，
   而**层外样式优先于任何层内样式，与特异性无关** —— 留在层外（本计划初稿如此）
   会让所有表单控件与按钮上的 `font-mono` 静默失效：模板输入框、变量芯片都算不出
   等宽字体栈，且没有任何报错。base 层早于 utilities 层，故工具类能正常覆盖它。
   紧随其后的光标规则**不**随之入层 —— 它的行为不依赖层归属，动它没有收益。 */
@layer base {
  button {
    font-family: inherit;
  }
...
<script setup>
// 发丝线容器，替代旧的「白卡片 + 一层浅阴影」。层级靠 12px 圆角 + 1px 发丝线
