...
- **窄屏的列宽收缩 + 隐去两个最低价值列**：这是 Review Focus 第 4 条的落点，做法分三层 ——
  1. `#` 列在 `<640px` 隐去（行号是装饰性信息），`解析剧名` 在 `<768px` 隐去（它的值已经在「新文件名」里体现）。**注意下面第 2 点的宽度数字已被实测推翻，见其后的更正。**
  2. 剩下的列在窄屏用**更小的宽度**（两个文件名列 `min-w-[140px] md:min-w-[220px]`，季/集 `w-[72px] md:w-[80px]`，状态 `w-[80px] md:w-[96px]`），`<table>` 的 `min-w` 相应从 `md:` 的 640px 降到 560px。
     窄屏下表宽**实测约 784px**（Task 7 在真实 Chromium 上量到 min-content 为 783.88px）。计划初稿按 `th` 的 `min-w` 算出 552px 是错的 —— auto 表格布局下真正决定列宽的是行标记的内容：`原文件名` 列因两处 `max-w-[320px] truncate` 占到 351.83px，`新文件名` 列因 `truncate`（nowrap）且无 `max-w` 占到 192.03px。**连带后果：`min-w-[560px]` 在 `sm` 以下是惰性的**（内容 min-content 已经超过它），而读到最右的「新文件名」需要**约 409px** 的横滑，不是 180px。要把它压到 552 必须改行标记，那是第三期 Task 5 的范围 —— 已记入该任务。
  3. `md:` 起恢复原宽度。此时各列宽度之和为 48(#) + 48(☑) + 220 + 160 + 80 + 80 + 96 + 220 ≈ **952px** —— 这就是 `md:` 档表格的实际横向下限（计划初稿写的 ≈900px 漏算了复选框列，是错的；Task 6 的实现在真实 Chromium 上量为 ≈952）（`<table>` 上的 `w-full` 在容器更宽时接管，所以不再给 `md:min-w` 一个会误导人的小数值）。
     直接后果：**视口窄于约 1180px 时，`md:` 档的表格也会横向滚动**（1024px 视口下工作区只有 1024 − 280 = 744px）。这不是回归 —— 改造前那 7 列同样有约 900px 的宽度需求，只是当时没有内部滚动容器去承接它。

  **本任务还要收掉 Task 5 留下的两处一次性兜底**（Task 5 的实现在矮视口修完裁剪后留下的 handoff）：`HomeView` 的 `<main>` 上那个 `overflow-y-auto`，以及传给 `<FileTable>` 的 `shrink-0`。两者都是为了让「旧卡片 + 480px 上限」在矮视口下仍可达而临时加的；本任务把 `FileTable` 改成 `flex-1` 并去掉 480px 上限后，它们既无必要、又会掩盖真实的收缩行为。**删完必须复测**：1024×600 与 900×620 两档，确认 `cardClippedPx = 0`、末行可达、`<main>` 不产生滚动条。
...
| 8 | 800×900 | 扫描文件 | 「解析剧名」列**可见**（它只在 `<768px` 隐藏，800 在 768 之上）；`#` 列仍在；季/集/状态列比 1280px 下更窄 |
| 9 | 375×812 | 扫描文件 | 表格横向可滚动；首屏看到「复选框 + 原文件名 + 季」；`解析剧名` 与 `#` 都不在 |
| 9b | 375×812 | 在 Console 里量 `document.querySelector('table').getBoundingClientRect().width` | **实测约 824px**（Task 6 的实现在真实 Chromium 上量到的值）。计划初稿按 `th` 的 `min-w` 算出 552px，**那个数是错的** —— auto 表格布局下，保留下来的行标记里两处 `max-w-[320px]` 会把「原文件名」列撑到约 352px，`th` 的 `min-w` 并非决定因素。要让 552 成立必须改行标记，那是第三期的范围 |
| 9c | 375×812 | 从最左滑到最右 | 约一次 180px 的横滑就能看到「新文件名」列，且该列在此宽度下**完整可见**（不要求「原文件名」同时可见 —— 这是已记录的取舍，见 Review Focus 第 4 条） |
| 9d | 1024×900 | 扫描文件并量 `document.querySelector('table').getBoundingClientRect().width` | 结果 ≈ **952px**，大于工作区的 744px，所以此档**也有横向滚动**。这是已知且有据的行为（改造前 7 列同样需要约 900px），不是本任务的缺陷；只要滚动发生在表格内部、整页仍不滚动即可 |
| 10 | 任意 | 点表头复选框 | 全选 / 全不选；行复选框状态同步 |
| 11 | 任意 | 修改某行的「集」 | 表格不重排（成立）。但**「新文件名」列不会更新** —— Task 6 的实现者抓取请求体后确认：`buildPreview()` 的载荷里有 `file_ids`/`source`/`path`/`template`/`folder_template`/`create_season_folder`，**没有 `overrides`**；而 `executeAction()` 有。于是 `updatePreview()` 写进 `row.override` 的值**只影响执行、不影响预览列** —— 用户看到的文件名与真正写入的不一致。**这是既存缺陷**（改造前的 `buildPreview` 亦然），已在第三期 Task 8 的计划里列为必修项（那一任务本就在改这两个请求体） |
...
**另外必须加一条「短视口」档（不是宽度档）**：Task 5 的评审指出 —— 中段容器是 `overflow-hidden` 而旧 `FileTable` 的卡片没有 `flex-1`、其内部滚动盒又被硬编码 `max-height: 480px` 卡住，所以在视口高度低于约 660px 时（1366×768 的笔记本减去浏览器 chrome 正落在这一区间）表格底部会被裁掉且无法滚到。**这是推演、不是实测**，所以本步要实测：用 1024×620 跑一次，量「表格底部能否到达 / 是否有滚动路径」。Task 6 把 FileTable 改成 `flex-1` + 去掉 480px 上限后会自然闭合，但本档要留下实测记录，不能靠「应该没问题」。

**另需知道（Task 7 实测）**：`(c)` 这条修法**前提不成立、不应应用** —— 375 下顶栏溢出为 0、品牌名宽 88.47px（被省略号截断而非零宽），应用它会在每台手机上藏掉应用名。`(d)` 的同一缺陷**在上一层仍然存在**：320×640 下 App.vue 根的轨道被顶栏撑到 367.4px（隐藏顶栏后轨道回到 320），且 375 只余 7.56px。故 `(d)` 的 `grid-cols-1` 需**同时**加到 App.vue 的根。
