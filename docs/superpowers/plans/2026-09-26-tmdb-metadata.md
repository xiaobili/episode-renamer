    ...
    async def _resolve_show(self, normalized: str, query: str):
        ...
        if tv_id is None:
            ...
            try:
                ...
                # 这里**刻意不与 detail 路径对称**, 别把它「修」回去。
                # 两条路径的 404 含义不同: get_tv_detail(tv_id) 的 404 确实意味着
                # 「那个 tv_id 不存在」→ show_not_found; 而 /search/tv 对不存在的剧
                # 是回 200 + 空结果, 所以它上面的 404 只可能来自基础设施
                # (代理 / DNS 屏蔽被拦截) → 归 unavailable。
                # 归成 show_not_found 会让代理屏蔽时预览显示「剧集未找到」,
                # 把用户推去核对文件名, 而真正的问题在网络。
                # 关键是不接住它会让预览请求 500, 违反「TMDB 故障绝不阻断重命名」。
...
5. 在「集」列手改一个不存在的集号（如 999）→ **点「预览」按钮** → 该行「TMDB」列应变为「无此集」，**其余行不受影响**。

   **关于 `{title}` 不要写成「渲染为空」—— 实测（实施阶段用一次性探针跑出来的）**：`updatePreview` 在**每一次**编辑时都会把四个字段（含 `title`，取自标题框的当前内容）整体写入该行的 `override`。所以只要用户碰过该行的任一字段，该行的 `title` 就成了一份「手动标题」——**无论他是否真的编辑过标题框**。于是：

   | 该行原本 | 改集号为 999 后 `{title}` |
   |---|---|
   | 有标题（如 `Breakage`） | 仍渲染 `Breakage` |
   | 无标题 / 用户清空了标题框 | 渲染为空 |

   两种都正确：前者是「手动标题优先」的应有之义。**不要**据此以为手动标题机制坏了。

   冻结的前提是**那一刻标题框非空** —— 空串走 `title: null`，TMDB 标题照样能进来，所以「原本无标题、只改集号」不冻结。对称地，**改标题也会把当时显示的 `show_name` / `season` / `episode` 一起冻结**：四个字段共用同一个 `@update:model-value` 处理器，无主次之分。这不是 bug，是「用户碰到这一行，就把这一行的四个字段都当手动值」这一种设计选择的后果 —— 知道它，才不会把正常行为当成故障去查。

   **注意这里必须点一下「预览」**：行编辑只写入 `row.override`，**不会**自动重建预览 —— 重建只由模板 / 补零位数 / 设置变更的 watch 与「预览」按钮触发。计划本可以要求「编辑即重建」，但那会给四个输入框的每次按键都带来一次服务端往返（整个文件列表的解析+渲染），属行为与性能改动，不在本计划范围。故验收以「点预览后生效」为准。
   （另注：`buildPreview` 直到实施阶段才补上 `overrides` 的下发 —— 在那之前，预览里的逐行编辑**根本不会反映到「新文件名」列**，本步骤当时不可能达成。）
