...
> **NFO 只跟随视频，不跟随字幕 —— 原文漏写，执行期补上。** 解析器对字幕与视频给出**完全相同**的结果（实测 `Show.S01E01.chs.srt` 与同名 `.mkv` 的解析结果一字不差），所以若不显式滤掉字幕：每个字幕旁会多写一个无意义的 `.nfo`（对 Emby/Jellyfin 无意义，**纯库污染**），而且决策按路径排序时会挂到字幕行上（`'….chs.srt' < '….mkv'`），使 `tvshow.nfo` 的落点显示在字幕行旁。判据用扫描器已设好的 `FileInfo.is_subtitle`，**不要**重新嗅探扩展名（那样会与 `settings.subtitle_extensions` 漂移）。**过滤只作用于 NFO 的构造，不影响字幕文件本身的重命名。**

`season.nfo` 与 `tvshow.nfo` 用的是**同一条规则的两种粒度**：前者按季取公共父，后者按剧取公共父。

### 9.1.1 既有的同名 NFO 跟随视频改名（执行期追加）

**问题**：`.nfo` **既不在** `video_extensions` 也不在 `subtitle_extensions`，所以它从不进入扫描、不在批次里，重命名路径上没有任何东西碰它。于是 `旧名 - S01E01.mkv` 被改名为 `新名 - S01E01.mkv` 之后，旁边的 `旧名 - S01E01.nfo` **必然留下** —— 一个与新文件名不符的孤儿。字幕没有这个问题（字幕在扫描里、会被当独立文件重命名），NFO 有。

**规则**：本地源重命名**视频**时，若**原文件名同前缀**的 `.nfo` 存在于原目录，把它一并移到新前缀。

四条边界，每条都有理由：

1. **只对视频生效，不对字幕** —— 字幕本就不该有 `.nfo`（那正是「NFO 只跟随视频」要防的垃圾）。
2. **只跟随同前缀的那一个** —— 绝不碰 `tvshow.nfo` / `season.nfo`（它们隶属剧/季，不隶属某个视频）。
3. **与「是否生成 NFO」无关** —— 它是**重命名**的一部分，不是 NFO 生成的一部分。主用例恰恰是「不生成 NFO、但已有 NFO 是别的工具写的」。
4. **顺序：先移动、后生成** —— 于是「生成开 + 覆盖关」时移过去的旧 NFO 占住新位置，生成报「已存在」，**用户原有的内容得以保留**；「覆盖开」时才被新内容替掉。这个顺序是关键：反过来会让生成先写、移动再覆盖它。

**两处失败处理**（沿用既有铁律）：目标 `.nfo` 已存在时**不覆盖**（报跳过，两个文件都不动 —— 宁可留下可见的孤儿，也不静默改写别人的元数据）；移动失败**不阻断批量**（重命名那时已成功）。

**dry-run 不移动**，但要**列出**将跟随移动的 NFO —— 与 §9.4 的可见性一致。

**不在本期**：OpenList 源。本期云盘不写 NFO（§2），故「跟随」在那里同样缺席 —— 这是已知限制，不是遗漏。
...
**剧集根 = 该剧所有选中视频新路径的公共父目录；若该目录本身是季目录，再上溯一层**，外加一道校验：

> **上溯那一步是执行期补上的，原文漏了。** 本节的三个例子**全是多季布局**，而**单季布局**（该剧的视频全在 `Season 02/` 一个目录里）从未被考虑 —— 此时「公共父目录」**就是那个季目录本身**，于是 `tvshow.nfo` 会落进 `Season 02/`，而 Emby/Jellyfin **只在剧集根**找它，等于白写。判据复用 `parser.py` 的 `_SEASON_DIR_PATTERNS`（`Season \d+` / `S\d+` / `第\d+季`），不另立一套。
>
> 三种季目录写法都要被识别；**未启用季文件夹**时目录名不匹配，不得上溯（那时剧集根与季目录本来就是同一层，见 §9.5）。
>
> **已知且被接受的上溯过头类别**：若**剧目录自身的名字**恰好匹配季目录模式（如库根下直接有一个 `Season 02/` 装着某剧的全部视频），上溯会把 `tvshow.nfo` 写到**库根**。执行期已证明**不存在纯路径判据**能区分它与「剧目录名恰好叫 Season 02」——`show_name` 与上溯后 root 的 basename 在两种布局下相等，而探测文件系统会破坏 `build_nfo_decisions` 的纯函数契约。可接受的理由有二：dry-run 的清单会在**执行前**把落点显示出来（§9.4 的可见性即安全阀），且该组合（剧目录名 = 季名 **且** 直接挂在库根下 **且** 批次里没有该库根下别的剧）罕见。上溯还是**单步而非循环**：`/m/Season 02/S01/` 会落到 `/m/Season 02/`，同一类别。
...
- **刮削失败要回退置位**：`refreshPreviewReportingFailure()` 返回 false 时把标志改回原值。
  否则一次 401 之后 TMDB 列会**一直**说「未启用」（把「Key 无效」说成「去设置页打开开关」），
  且后续每次自动回刷（如改模板）都继续带着被拒的 Key 重试。
- **在途的刮削视为「尚未刮削」**：两个谓词把 `scraping` 也作为输入
  （`tmdbPendingScrape` = `(!scraped || scraping) && !tmdbDisabled`；`nfoBlockedByScrape` 同理）。
  理由：在刮削返回之前，屏幕上那张表**确实**还是刮削前的那一张 —— 此时说「未启用」是误诊断
  （把用户推去翻一个本来就开着的开关），而说「未刮削」是实话。
...
- **刮削在途时其它动作一并门控**（`预览` / `清空` / `试运行` / `执行重命名`）：`scraped` 在 await
  **之前**就置位，而屏幕上的表还是刮削前那一张 —— 此时发起执行会写出**用户没见过的文件名**，
  正是 §17.6 与 R29/R52/R54 一直在防的「预览与执行不一致」。实现方式：把 `scraping` 传进
  `AppBottomBar` 与 `FileTable`，并入各自按钮的禁用条件（与既有的 `executing` 同一手法）。
- 禁用条件：无文件 / 正在扫描。
...
| 标题列 | 空（正确：它就是个空输入框） |
| TMDB 列 | **「未刮削」** |
| NFO 列 | **「未刮削」**；勾了「生成 NFO」时左栏另给一条提示：NFO 需要 TMDB 元数据，未刮削则不会写出（**云盘源下不显示** —— 与相邻那条 NFO 说明一致：云盘本就不写 NFO，提示在那里是噪音） |
