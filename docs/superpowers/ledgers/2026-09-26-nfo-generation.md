...
裁定编号 **R60–R93**（R1–R59 属于计划 1，见 `2026-09-26-tmdb-metadata.md`）。
...
§9.1.1 实现后经一次任务审查（Approved，1 Important + 2 项）与一轮修复，最终 **214 passed**
（含无网命名空间），**§9.1.1 修复轮的再审查判「全部 ADDRESSED，无新 Critical/Important」**。

**[已被 R91 推翻 —— 顶行已改「项」；本条保留仅供追溯。]** Ruling R89: 审查者指出修复轮把**跳过计数从「路径数」变成了「动作数」** —— 同一路径上
「生成没写」与「跟随没搬成」两条理由并存时会计 2，顶行「跳过 2 个」把文件数说过了一个。
**裁定：接受（不修）。** 理由：紧随其下的按理由分组会把数字对上（「已存在 1 个；旧前缀 NFO 未跟随… 1 个」），
故它不是谎话，只是顶行没说什么是一个「个」。**代价若判错**：用户可能把动作数读成文件数；
最小的改法是顶行改「项」或只计去重路径。**留给下一个会话决定。**

**[已闭合于 R92。]** Ruling R90: 实现报告里一处**陈旧的理由串**（仍写着修复前的 `同名 NFO 未跟随移动: 目标 NFO 已存在, 不覆盖`，
与代码及报告 §9.2 矛盾）。**裁定：不修**（纯报告内部不一致，代码正确）。**代价若判错**：读报告的人
会以为存在一个已经不存在的理由串。**留给下一个会话决定。**

### 期末状态（供接手）

- 分支 `feat/tmdb-nfo`，工作树干净。
- 后端 **214 passed**，密闭（无网命名空间复跑过）。
- 前端：`check-settings-schema.mjs` / `check-sfc-compile.mjs` / `npm run build` 三项通过；
  **无前端测试框架**（仓刻意如此），故前端行为只有机械检查 + 人工走查。
- 构建产物已由 `f8cdfa4` 提交（两期合并为一次，含 §9.1.1 的跟随 UI）；R91 的单位改动由 `a40267e` 更新。
- **已知限制（不是遗漏）**：
  1. `/rename/preview` 不知道跟随（它只从 `build_nfo_decisions` 建落点），故生成关闭时预览对 NFO
     一言不发而执行仍会移动文件。§9.1.1 的 dry-run 条款由 `/rename/dry-run` 满足。
  2. OpenList 源不写 NFO、也不跟随（本期范围）。
  3. 前端四条修复与 §9.1.1 的渲染无常驻自动化判据。
  4. `_show_root` 的过度上溯类别（R76）—— 已文档化、接受。
- ~~**待办（交给下一个会话）**：写「裁定清单」（R1–R90 里会改变行为/范围的那些，附判错代价）；
  上一条消息已给出该清单应含哪几类的索引。~~ **已完成（见 R93）：`docs/superpowers/ledgers/2026-09-26-tmdb-nfo-ruling-index.md`。**
  原文提到的「上一条消息给出的分类索引」随上个会话消失，分类规则由本会话重推并写在清单开头（可核）。

---

## 下一会话的裁定（R91–R92，收尾 R89/R90）与裁定清单（R93）

Ruling R91: **R89 逆转 —— 顶行与按理由分组的分项同改单位「项」。** 上一会话把决定留给本会话，本会话判：**改**。
  理由：① R89 自己的辩护是「紧随其下的按理由分组会把数字对上」—— 但那个辩护**只在下行也说「项」时成立**：
  分组按条目计，若顶行「个」而下行「项」，数字照样对得上、单位却不一致；两边同改才让「顶行 = 分项之和」
  这件事真正可见。② **未选「只计去重路径」**：那会让顶行（1）小于分项之和（2），读者更容易当成 bug；
  而「个」在相邻的「写入 N 个」「跟随移动 N 个」上是真文件数，混同两处的代价大于统一。
  落地：`df0ecdb`（源码）+ `a40267e`（产物）。
  **代价若判错**：用户把「跳过 2 项」仍读成 2 个文件（此时 2 项 = 1 个文件）—— 概率低于改前，因为
  「项 ≠ 个」本身就是区分提示；且该场景罕见（需目标 NFO 已存在**且**旧前缀孤儿同时存在）。回退成本 = 一个字 + 一次构建。
  **这条改动无常驻自动化判据**（spec §17 #3：无前端测试框架）。产物级判据（旧产物 HomeView chunk「项」0 处
  → 新产物 2 处）证明的是「产物携带了这次改动」，**不是**「渲染在浏览器里正确」；后者仍列 deferred-to-human。
  另一条已核的边界：全仓只有 `ResultDialog.vue` 渲染这几个计数（`grep nfoStats|nfo_skipped|skippedCount`），
  不存在第二处需要同步的渲染点。

Ruling R92: **R90 闭合 —— 无物可修。** 核实过程：`git log -S "同名 NFO 未跟随移动" --all` 只命中三个提交
  —— `a90cfbc`（把该串引入**代码与测试**：`local_renamer.py:60`、`test_nfo_write.py:704`）、`8f2a1d0`
  （两处改成现行串 `旧前缀 NFO 未跟随: 新名字处已有文件, 覆盖开关不影响跟随`）、`a3f9bdf`（即 R90 自己的文字）。
  R90 所说的「实现报告」是收尾时随 `.superpowers/sdd/2026-09-26-nfo-generation/` 一同删除的 scratch 工件，
  **从不入库**（该目录自带 `.gitignore`）；当前全仓检索，该陈旧串只存在于 R90 本条文字里，且那处已明写「修复前的」。
  **裁定：闭合，不修。** 代码与测试的现行串一致（`local_renamer.py:64` ↔ `test_nfo_write.py:717`）。
  **代价若判错**：若将来有人从会话记录或剪贴板重新引用该串，会以为它是现行值 —— 唯一防线就是 R90/R92 这两条文字自身。

Ruling R93: **裁定清单已写出**：`docs/superpowers/ledgers/2026-09-26-tmdb-nfo-ruling-index.md`，
  覆盖 R1–R93 里**会改变行为或范围**的裁定（机械数出：实体 44 个条目 + 附录 28 个条目 = 72 个条目，
  触及 77 个裁定号；未触及的 16 个逐条列在清单末尾），每条给「现行行为 / 判错代价 / 证据锚点」；
  分类规则写在清单开头（本会话重推，因原文引用的上个会话消息已失）。
  **顺带发现一处账目缺口**：R1–R93 里 **R80 没有正文**（92 条有正文）—— 它只出现在提交 `384801f`
  的标题里；清单把它的内容归到 R82 并在 H 节记明。**代价若判错**：分类是我一个人做的，
  若审查者认为某条该进实体或该移出，改的是分类而不是事实（每条都带锚点，可直接核）。
