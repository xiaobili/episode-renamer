...
def _show_root(paths: list[str]) -> str:
    ...
    **已知的过度上溯（同一类别, 没有纯路径判据 —— 探过了, 别再试）**：
    当**剧目录自身的名字**也匹配季目录模式、且它直接挂在库根下时, 这一步会把剧集根
    抬到**库根**：`/m/lib/Season 02/x.mkv` → `tvshow.nfo` 落在 `/m/lib/tvshow.nfo`。
    若 `/m/lib` 正是 Emby 的库根, 那就是 §9.2 的不可逆污染, 而混放守卫抓不到它 ——
    守卫只看**本批次**的条目, 而「只重命名一部剧」是常见流程。

    **为什么不在代码里堵**: 不存在纯路径判据。
    ①季目录判定只看一层, 看不见「它的父目录名字也是季名」;
    ②最顺手的补救 —— 要求 `show_name == basename(上溯后的 root)` 才上溯 —— 经探针
    验证**无法区分**这两种布局: 解析器推导剧名时会跳过季目录, 于是坏布局
    （`/m/lib/Season 02/`, 剧名 `lib`）与合法布局（`/m/绝命毒师/Season 02/`, 剧名
    `绝命毒师`）**都**满足该等式（均为 True）, 判据对两者一视同仁;
    ③`show_name` 本身也不是路径的函数 —— 用户可手动覆盖剧名（`overrides` /
    `apply_override`）, 同一路径换个剧名就能让任何基于它的判据翻面;
    ④去看文件系统里有没有别的剧, 会破坏本模块「纯函数、不碰 FS」的契约, 连带毁掉
    它可被穷举测试的性质。

    **为什么可以接受**: ①dry-run 的清单在**执行前**把这个落点显示出来
    （`/m/lib/tvshow.nfo` 用户看得见）—— 那是实际起作用的安全阀;
    ②这个组合（剧目录名恰好是季名 **且** 直接挂在库根下）很罕见。
    ③上溯是**单步而非循环**: `/m/Season 02/S01/x.mkv` 落到
    `/m/Season 02/tvshow.nfo`（落在另一个季形目录里）—— 同一类别, 刻意不 while,
    写在这里免得后来者以为漏了。
    """
